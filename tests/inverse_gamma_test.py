...
class TestInverseGamma():
    ...
    natural_params = inverse_gamma_natural_from_standard(standard_params)
    mean_params = inverse_gamma_mean_from_natural(natural_params)

    def test_natural_params_mean_params_consistency(self):
        natural_params2 = inverse_gamma_natural_from_mean(self.mean_params)
    ...
    def test_log_partition_mean_params_consistency(self):
        mean_params2 = grad(inverse_gamma_log_partition)(self.natural_params)
    ...
    def test_sample_mean_params_consistency(self):
        nsamples = 10000
        x, self.rng = rngcall(inverse_gamma_sample, self.rng, self.natural_params, (nsamples,))
        stats = vmap(inverse_gamma_stats)(x)
    ...
    def test_sample_logprob_entropy_consistency(self):
        nsamples = 10000
        x, self.rng = rngcall(inverse_gamma_sample, self.rng, self.natural_params, (nsamples,))
        logprob = vmap(partial(inverse_gamma_log_prob, self.natural_params))(x)
        entropy = inverse_gamma_entropy(self.natural_params)
